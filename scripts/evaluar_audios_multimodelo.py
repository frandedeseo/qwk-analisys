        ...
        try:
            with open(archivo_audio, 'rb') as f:
                ...
                if response.status_code == 200:
                    ...
                    print(f"  ✗ Error {response.status_code} para {id_lectura}: {response.text[:100]}")
...
def ejecutar_evaluaciones():
    ...
    print(f"Configuración: {len(MODELOS)} modelos × 4 grados × 8 audios = {len(MODELOS) * 4 * 8} evaluaciones\n")
        ...
        for grado in range(1, 5):
            ...
            for idx, archivo_audio in enumerate(audios, 1):
                ...
                if id_lectura is None:
                    print(f"    ✗ [{idx}/8] No se pudo extraer ID de {archivo_audio.name}")
                    continue
                
                print(f"    ⏳ [{idx}/8] ID {id_lectura}...", end=" ")
